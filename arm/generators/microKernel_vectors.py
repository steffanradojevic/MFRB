...
class microKernel:
    ...
    def generate_loop_body(self):
        ...
        # Store loop counter in v0.4.s for the writeKernel
        if self.kernel == "writeKernel":
            code.append(f"\tscvtf s0, x1\n")
            code.append(f"\tfmov w30, s0\n")
            code.append(f"\tdup v0.4s, w30 // v0.4s (st1) == q0 (str)\n")
        ...
        for stride in range(self.strides):
            ...
            for portion in range(self.portions):
                ...
                # Generate memory access for readKernel LD1 and LDR vector instructions
                if self.kernel == "readKernel":
                    if self.instr_type == "ld1":
                        code.append(
                            f"\tld1 {chr(123)} v{vreg_idx}.4S {chr(125)}, [{self.stride_registers[stride]}], #16 //array id: {int(array_id_value/4)} bytes={array_id_value}\n"
                        )
                    elif self.instr_type == "ldr":
                        code.append(
                            f"\tldr q{vreg_idx}, [{self.stride_registers[stride]}], #16 //array id: {int(array_id_value/4)} bytes={array_id_value}\n"
                        )

                # Generate memory access for writeKernel ST1 and STR vector instructions
                elif self.kernel == "writeKernel":
                    if self.instr_type == "st1":
                        code.append(
                            f"\tst1 {chr(123)} v0.4S {chr(125)}, [{self.stride_registers[stride]}], #16 //array id: {int(array_id_value/4)} bytes={array_id_value}\n"
                        )
                    elif self.instr_type == "str":
                        code.append(
                            f"\tstr q0, [{self.stride_registers[stride]}], #16 //array id: {int(array_id_value/4)} bytes={array_id_value}\n"
                        )

                vreg_idx += 1  # Point to next vector register
                portion_offset += 16  # Point to next portion unroll
