...
class microKernel:
    ...
    def generate_prologue(self):
        ...
        code.append(f"\t.text \n")
        code.append(f"\t.global microKernel # Scalar\n")
        code.append(f"\n")
        code.append(
    ...
    def generate_loop_body(self):
        ...
        # Store loop counter in f0 for the writeKernel
        if self.kernel == "writeKernel":
            code.append(f"\tfcvt.s.w f0, x3 \n")  # cast loop counter to float
        ...
        for stride in range(self.strides):
            ...
            for portion in range(self.portions):
                # Reset freg (float register) to 0 when exceeding freg 31
                if freg_idx > 31:
                    freg_idx = 0
                ...
                # Generate memory access for readKernel flw scalar instructions
                if self.kernel == "readKernel":
                    code.append(
                        f"\tflw f{freg_idx}, {portion_offset}({self.stride_registers[stride]}) #array id: {int(array_id_value/4)} bytes={array_id_value}\n"
                    )

                # Generate memory access for writeKernel fsw scalar instructions
                elif self.kernel == "writeKernel":
                    code.append(
                        f"\tfsw f0, {portion_offset}({self.stride_registers[stride]}) #array id: {int(array_id_value/4)} bytes={array_id_value}\n"
                    )

                freg_idx += 1  # Point to next float register
                portion_offset += 4  # Point to next portion unroll

        if self.formatting:
            code.append(f"\n")
            code.append(f"# update stride address registers\n")

        # Offset encoded within a instruction, stride memory addresses have to point to next loop
        stride_update = int(self.portions * 4)
        idx_reg = 0
        for stride in range(self.strides):
            code.append(
                f"\taddi {self.stride_registers[stride]}, {self.stride_registers[stride]}, {stride_update}\n"
            )
    ...
    def split_immediate_lui_addi(self, offset):
        ...
        copy_offset = (
            offset  # don't know if bitshifting changes something, just to be sure
        )
        lui_offset = copy_offset >> 12

        if (
            0b100000000000 & offset
        ) != 0:  # 12bit is set and we have to to it a bit differently
